 ...
 * For each client, it handles exactly 1 HTTP transaction.
        most likely changing this function to handle multiple clients concurrently
        how would we do this?
        should we include threadpool.h to call upon the methods directly?
 */
static void
server_loop(char *port_string)
{
    ...
    while (accepting_socket != -1) {
        ...
        /* Processes multiple thread for multiple client support */
        pthread_t thread;
        int create_val = pthread_create(&thread, NULL, thread_body, bufio_create(client_socket));
        pthread_detach(thread);
        if (create_val != 0) {
            perror("Issue with creating thread");
        }
    }
}

/* Process that each thread should complete */
static void * thread_body(void *arg) { 
